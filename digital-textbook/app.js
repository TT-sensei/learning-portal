...
const PLATFORMS = {
  manaviewer: {
    name: 'まなビューア',
    short: 'MV',
    description: '公式ユーザーログイン',
    url: 'https://manaviewer.jp/'
  },
  cho: {
    name: '超教科書',
    short: '超',
    description: '公式教材利用画面',
    url: 'https://p01.cloud.cho-textbook.jp/'
  },
  lentrance: {
    name: 'Lentrance Reader',
    short: 'LR',
    description: '公式学校ログイン',
    url: 'https://www.lentrance.com/school/login'
  },
  mirai: {
    name: 'みらいスクール',
    short: 'MS',
    description: '公式ユーザーログイン',
    url: 'https://mirai-pf.jp/user/login.html'
  },
  tsubasa: {
    name: 'つばさブック',
    short: 'TB',
    description: '公式ユーザーログイン',
    url: 'https://tsubasabook.jp/'
  },
  esviewer: {
    name: 'エスビューア',
    short: 'SV',
    description: '公式ユーザーログイン',
    url: 'https://sviewer.jp/'
  },
  custom: {
    name: 'その他のログイン先',
    short: '＋',
    description: 'URLを指定して登録',
    url: ''
  }
};
...
function platformKeyFor(link) {
  return PLATFORMS[link.platform] ? link.platform : 'custom';
}

function platformInfo(link) {
  return PLATFORMS[platformKeyFor(link)] ?? PLATFORMS.custom;
}
...
function normalizeLink(raw) {
  ...
  const publisher = String(raw.publisher || '').trim().slice(0, 40);
...
function sanitizeState(raw) {
  ...
  const recent = Array.isArray(raw?.recent)
    ? raw.recent.filter((item) => ids.has(item.id) && Number.isFinite(Number(item.time))).slice(0, 20).map((item) => ({ id: item.id, time: Number(item.time) }))
    : [];
  return { links, recent };
}
...
function fallbackStorage() {
  ...
  return {
    ...
    getAll() {
      return { [STATE_KEY]: this.load(STATE_KEY, null) };
    }
  };
}
...
async function initStorage() {
  try {
    ...
    StorageManagerClass = module.StorageManager;
    storage = new StorageManagerClass(STORAGE_NAMESPACE);
  } catch {
    // edu-componentsへ接続できない環境でもランチャー自体は動作させる。
...
function formatRecentTime(time) {
  ...
  const diff = Math.round((now - date) / day);
  if (diff === 1) return '昨日';
  if (diff < 7) return `${diff}日前`;
  return `${date.getMonth()+1}/${date.getDate()}`;
}

function defaultLabel(subject, publisher, platform) {
  if (publisher) return `${subject}　${publisher}`;
  return `${subject}　${PLATFORMS[platform]?.name || 'デジタル教科書'}`;
}
...
function sortedLinks() {
  return [...state.links].sort((a, b) => Number(b.favorite) - Number(a.favorite) || a.subject.localeCompare(b.subject, 'ja') || a.label.localeCompare(b.label, 'ja'));
}
...
function renderLinks() {
  ...
  for (const link of links) {
    const platform = platformInfo(link);
    ...
    card.innerHTML = `
      ...
      <h3>${escapeHtml(link.label || defaultLabel(link.subject, link.publisher, link.platform))}</h3>
...
function openDialog(platform = null, link = null) {
  ...
  customUrlInput.value = platform === 'custom' ? '' : (link?.url && !PLATFORMS[link?.platform]?.url ? link.url : '');
  if (platform === 'custom') customUrlInput.value = '';
...
function addOrUpdateLink(event) {
  ...
  const url = validUrl(custom ? customUrlInput.value.trim() : PLATFORMS[platform]?.url || '');
  if (!subject || !url) {
    showToast(custom ? '教科と正しいログインURLを入力してください。' : '教科を選んでください。');
    return;
  }
  const item = {
  ...
  const existingIndex = state.links.findIndex((link) => link.id === item.id);
  if (existingIndex >= 0) state.links[existingIndex] = item;
  else state.links.push(item);
  saveState();
  renderAll();
  closeDialog();
  showToast(existingIndex >= 0 ? '教科の設定を更新しました。' : '教科を登録しました。');
}

function removeLink(id) {
  const link = state.links.find((item) => item.id === id);
  if (!link) return;
  const ok = window.confirm(`「${link.label || link.subject}」を登録から削除しますか？`);
  if (!ok) return;
...
function moveLink(id, direction) {
  // 「お気に入りを優先表示」を保ちつつ、同じ表示グループ内の順序を変える。
  const links = sortedLinks();
  ...
  [current.favorite, target.favorite] = [target.favorite, current.favorite];
  const a = state.links.findIndex((item) => item.id === current.id);
  const b = state.links.findIndex((item) => item.id === target.id);
  if (a >= 0 && b >= 0) [state.links[a], state.links[b]] = [state.links[b], state.links[a]];
  saveState();
  renderLinks();
}
...
function showToast(message) {
  ...
  clearTimeout(recentClock);
  recentClock = setTimeout(() => toast.classList.remove('show'), 2200);
}
...
async function importSettings(file) {
  try {
    ...
    if (payload?.type && payload.type !== 'tt-sensei-digital-textbook-launcher') throw new Error('別の形式');
    const next = sanitizeState(payload?.state ?? payload);
    state = next;
    saveState();
    renderAll();
...
function resetSettings() {
  const ok = window.confirm('この端末に保存した教科登録・お気に入り・最近の履歴をすべて初期化しますか？');
  if (!ok) return;
  state = { links: [], recent: [] };
  storage?.clear?.();
  saveState();
...
linkGrid.addEventListener('click', (event) => {
  ...
  if (action === 'edit') openDialog(null, link);
  if (action === 'move-up') moveLink(id, -1);
  if (action === 'move-down') moveLink(id, 1);
});
